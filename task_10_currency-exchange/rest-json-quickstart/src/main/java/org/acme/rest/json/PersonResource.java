...
public class PersonResource {
    ...
    public PersonResource() {
        ...
        persons.add(new Person("1", "Ivan", "IvAn", "email", address, number, "site", company, 1000, "USD"));
        persons.add(new Person("2", "Ivan", "IvAn", "email", address, number, "site", company, 1000, "EUR"));
    }
    ...
    @GET
    @Path("/{id}/currency-exchange/{currency}")
    public Integer currency_exchange(@PathParam("id") String id, @PathParam("currency") String currency) {
        Person person = persons.stream().filter(p -> p.getId().equals(id)).findFirst().get();
